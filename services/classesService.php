...
class classesService implements IDao {
    ...
    public function select($IdFiliere) {
        $query = "SELECT classes.id, classes.code, classes.IdFiliere FROM classes inner join filiere on filiere.id = classes.IdFiliere WHERE filiere.code=?";
        $req = $this->connexion->getConnexion()->prepare($query);
        $req->execute( array($IdFiliere)) or die('Error');
